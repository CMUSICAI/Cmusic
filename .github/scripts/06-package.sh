...
cd ${GITHUB_WORKSPACE}
PKGVERSION=`grep "PACKAGE_VERSION" src/config/cmusicai-config.h | cut -d\" -f2`
VERSION="${PKGVERSION}"
...
if [[ ${OS} == "windows" ]]; then
    ...
    for rmfile in detach-sig-create.sh win-codesign.cert cmusicai-cli.exe Cmusic-Qt.exe cmusicaid.exe; do
        if [[ -e ${rmfile} ]]; then
            rm -f ${rmfile}
        fi
    done
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/Cmusic-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/Cmusic-Qt.app/Contents/MacOS/install_cli.sh
    fi
