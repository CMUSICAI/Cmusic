...
The RPM spec file provided here is for CmusicAI-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://cmusic.ai//bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[CmusicAI git repository](https://github.com/CMUSICAI/CmusicAI-master) but are not part of
...
Currently only `Source10` is used, for the example `cmusicai.conf` file.
...
in the cmusicai git repository. At present only an SVG version of the CmusicAI
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the cmusicai
...
needed when 0.12.1 is released, a proper fix is already in the CmusicAI git
master branch.
...
idea to build CmusicAI Core against a static version of that library compiled
...
The `cmusicai.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `cmusicai-server` package. At this point, most RPM based
...
The `cmusicai-server` package should have SELinux support. How to properly do
...
CmusicAI-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/cmusicai/). He has been
packaging CmusicAI for Fedora at least since 2012.
...
4. I renamed the `cmusicai` package that contains the Qt GUI to `cmusicai-core` as
that appears to be how the general population refers to it, in contrast to
`cmusicai-xt` or `cmusicai-classic`. I wanted to make sure the general population
