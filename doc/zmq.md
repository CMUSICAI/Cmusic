...
The CmusicAI Core daemon can be configured to act as a trusted "border
router", implementing the cmusicai wire protocol and relay, making
...
The ZeroMQ feature in CmusicAI Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building cmusicaid:
...
For instance:
    ...
    $ cmusicaid -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/cmusicaid.tx.raw
...
These options can also be provided in cmusicai.conf.
...
From the perspective of cmusicaid, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into cmusicaid directly. Furthermore, no information is
...
using. CMUSICAId appends an up-counting sequence number to each
