...
The CMUSICAIcoin Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play cmusicais" on the test network, if you
...
CmusicAI Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- CMUSICAIMiner : Generates cmusicais (if wallet is enabled).
...
However, in open source software such as CmusicAI Core, where everyone uses
...
pay attention to for reviewers of CmusicAI Core code.

General CmusicAI Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: CmusicAI Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against CMUSICAIcoin so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:cmusicai/cmusicai.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `cmusicai-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `cmusicai-cli` and the GUI debug console use this table to determine how to
