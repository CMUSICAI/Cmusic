...
#include "cmusicaigui.h"

#include "cmusicaiunits.h"
...
const std::string CmusicAIGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString CmusicAIGUI::DEFAULT_WALLET = "~Default";

CmusicAIGUI::CmusicAIGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void CmusicAIGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a cmusicai address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and cmusicai: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to cmusicai addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your cmusicai addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified cmusicai addresses"));
    ...
    openAction->setStatusTip(tr("Open a CmusicAI: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible cmusicai command-line options").arg(tr(PACKAGE_NAME)));
...
void CmusicAIGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void CmusicAIGUI::createToolBars()
{
    if(walletFrame)
    {
        /** CMS START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/cmusicaicointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* cmusicaiLabelLayout = new QVBoxLayout(toolbarWidget);
        cmusicaiLabelLayout->addWidget(label);
        cmusicaiLabelLayout->addWidget(toolbar);
        cmusicaiLabelLayout->setDirection(QBoxLayout::TopToBottom);
        cmusicaiLabelLayout->addStretch(1);
        ...
        // Get the latest CmusicAI release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void CmusicAIGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool CmusicAIGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool CmusicAIGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void CmusicAIGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void CmusicAIGUI::setWalletActionsEnabled(bool enabled)
{
...
void CmusicAIGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void CmusicAIGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void CmusicAIGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CmusicAIGUI::optionsClicked()
{
...
void CmusicAIGUI::showDebugWindow()
{
...
void CmusicAIGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void CmusicAIGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void CmusicAIGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void CmusicAIGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void CmusicAIGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void CmusicAIGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void CmusicAIGUI::openClicked()
{
...
void CmusicAIGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void CmusicAIGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void CmusicAIGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CmusicAIGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CmusicAIGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CmusicAIGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** CMS START */
void CmusicAIGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void CmusicAIGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void CmusicAIGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void CmusicAIGUI::gotoRestrictedAssetsPage()
{
...
void CmusicAIGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to cmusicai network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void CmusicAIGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void CmusicAIGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void CmusicAIGUI::updateHeadersSyncProgressLabel()
{
...
void CmusicAIGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void CmusicAIGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("CmusicAI"); // default title
...
void CmusicAIGUI::changeEvent(QEvent *e)
{
...
void CmusicAIGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void CmusicAIGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void CmusicAIGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "cmusicai")
        msg += tr("Amount: %1\n").arg(CmusicAIUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(CmusicAIUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void CmusicAIGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CmusicAIGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool CmusicAIGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool CmusicAIGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CmusicAIGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void CmusicAIGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void CmusicAIGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void CmusicAIGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CmusicAIGUI::detectShutdown()
{
...
void CmusicAIGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void CmusicAIGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void CmusicAIGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CmusicAIGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(CmusicAIGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void CmusicAIGUI::subscribeToCoreSignals()
{
...
void CmusicAIGUI::unsubscribeFromCoreSignals()
{
...
void CmusicAIGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void CmusicAIGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<CmusicAIUnits::Unit> units = CmusicAIUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const CmusicAIUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(CmusicAIUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(CmusicAIUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (CmusicAIUnits::Unit u : CmusicAIUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(CmusicAIUnits::name(u)), this);
...
{
    setText(CmusicAIUnits::name(newUnits));
}
...
void CmusicAIGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=cmusicaiBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void CmusicAIGUI::mnemonic()
{
