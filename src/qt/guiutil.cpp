...
#include "cmusicaiaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a cmusicai address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new CmusicAIAddressEntryValidator(parent));
    widget->setCheckValidator(new CmusicAIAddressCheckValidator(parent));
}
...
bool parseCmusicAIURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no cmusicai: URI
    if(!uri.isValid() || uri.scheme() != QString("cmusicai"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CmusicAIUnits::parse(CmusicAIUnits::cmusicai, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCmusicAIURI(QString uri, SendCoinsRecipient *out)
{
    // Convert cmusicai:// to cmusicai:
    //
    //    Cannot handle this later, because cmusicai:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("cmusicai://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "cmusicai:");
    }
    QUrl uriInstance(uri);
    return parseCmusicAIURI(uriInstance, out);
}

QString formatCmusicAIURI(const SendCoinsRecipient &info)
{
    QString ret = QString("cmusicai:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CmusicAIUnits::format(CmusicAIUnits::cmusicai, info.amount, false, CmusicAIUnits::separatorNever));
        paramCount++;
    }
...
bool openCmusicAIConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", CMUSICAI_CONF_FILENAME));

    /* Open cmusicai.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CmusicAI.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CmusicAI (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("CmusicAI (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for CmusicAI*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "cmusicai.desktop";
    return GetAutostartDir() / strprintf("cmusicai-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a cmusicai.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=CmusicAI\n";
        else
            optionFile << strprintf("Name=CmusicAI (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the cmusicai app
...
bool GetStartOnSystemStartup()
{
    CFURLRef cmusicaiAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (cmusicaiAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, cmusicaiAppUrl);

    CFRelease(cmusicaiAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add cmusicai app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, cmusicaiAppUrl, nullptr, nullptr);
    }
