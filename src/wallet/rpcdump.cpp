...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CmusicAI address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The cmusicai address for the private key\n"
            "\nResult:\n"
    ...
    return CCmusicAISecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to cmusicaid)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/CMUSICAI/CmusicAI-master/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by CmusicAI %s\n", CLIENT_BUILD);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                CCmusicAIExtKey b58extkey;
                b58extkey.SetKey(masterKey);

                CExtPubKey pubkey;
                pubkey = masterKey.Neuter();

                CCmusicAIExtPubKey b58extpubkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CCmusicAISecret(key).ToString(), strTime);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                // Get the CmusicAI Ext Key from the master key
                CCmusicAIExtKey b58extkey;
                b58extkey.SetKey(masterKey);
                ...
                // Get the CmusicAI Ext Key from the public key
                CCmusicAIExtPubKey b58extpubkey;
                b58extpubkey.SetKey(pubkey);
                ...
                // Create the CmusicAI Account Ext Private Key
                CCmusicAIExtKey b58accountextprivatekey;
                b58accountextprivatekey.SetKey(accountKey);

                // Create the CmusicAI Account Ext Public Key
                CCmusicAIExtPubKey b58actextpubkey;
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see cmusicaid log for details) and could "
