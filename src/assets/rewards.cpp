...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for cmusicai
    ...
    if (p_rewardSnapshot.strDistributionAsset != "cmusicai") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is cmusicai with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using cmusicai differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "cmusicai") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse CmusicAI address (already validated during ownership snapshot creation)
