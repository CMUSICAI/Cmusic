 ...
 * between cmusicaid, and cmusicai-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateCmusicAIs(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), CMUSICAI_CONF_FILENAME));
    if (mode == HMM_CMUSICAID)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), CMUSICAI_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/CMUSICAI/CmusicAI-master>";
    const std::string URL_WEBSITE = "<https://cmusic.ai/>";
...
/** Sanity checks
 *  Ensure that CmusicAI is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single CmusicAI process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", CMUSICAI_CONF_FILENAME)).string());
    ...
    GenerateCmusicAIs(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
