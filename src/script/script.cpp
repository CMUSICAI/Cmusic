...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** CMS START */
    case OP_cmusicai_ASSET              : return "OP_cmusicai_ASSET";
    /** CMS END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_cmusicai_ASSET) { // OP_cmusicai_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == cmusicai_N) { // Check to see if cmusicai starts at 27 ( this->size() < 105)
                if ((*this)[28] == cmusicai_E)
                    if ((*this)[29] == cmusicai_X)
                        index = 30;
            } else {
                if ((*this)[28] == cmusicai_N) // Check to see if cmusicai starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == cmusicai_E)
                        if ((*this)[30] == cmusicai_X)
                            index = 31;
            }

            if (index > 0) {
                nStartingIndex = index + 1; // Set the index where the asset data begins. Use to serialize the asset data into asset objects
                if ((*this)[index] == cmusicai_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == cmusicai_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == cmusicai_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == cmusicai_N) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_cmusicai_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_cmusicai_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_cmusicai_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_cmusicai_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_cmusicai_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_cmusicai_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
