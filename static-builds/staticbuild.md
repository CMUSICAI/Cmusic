...
sudo find ${DISTNAME}/bin -type f -executable -exec ../cmusicaicoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../cmusicaicoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo cmusicai-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find cmusicai-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/cmusicaicoin
...
cp -f cmusicai-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf cmusicaicoin/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/cmusicaicoin/cmusicai-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/cmusicai-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer cmusicai-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer cmusicai-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf cmusicaicoin/contrib/windeploy ~/sign/win32
...
#transfer cmusicai-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/cmusicai-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer cmusicai-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
