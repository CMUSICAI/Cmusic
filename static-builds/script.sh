...
find ${DISTNAME}/bin -type f -executable -exec ../CmusicAI/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../CmusicAI/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
cd ~/
rm -rf win64
cp -rf CmusicAI/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir -p unsigned
mv ~/CmusicAI/cmusicai-*setup-unsigned.exe unsigned/
...
cd ~/
rm -rf win32
cp -rf CmusicAI/contrib/windeploy ~/sign/win32
...
mkdir -p ~/CmusicAI/depends/SDKs
cp ~/MacOSX10.11.sdk.tar.gz ~/CmusicAI/depends/SDKs/MacOSX10.11.sdk.tar.gz
cd ~/CmusicAI/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
