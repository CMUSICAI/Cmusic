...
`test/functional/test_framework/test_framework.py:CmusicAITestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/cmusicai_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    logging.debug("Temporary test directory at %s" % tmpdir)

    # Don't run tests on Windows by default
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/CMUSICAI/CmusicAI-master/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/Bitcoin/bitcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    # Check that the build was configured with wallet, utils, and cmusicaid
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_cmusicaid = config["components"].getboolean("ENABLE_CMUSICAID")
    if not (enable_wallet and enable_cli and enable_cmusicaid):
        print("No functional tests to run. Wallet, utils, and cmusicaid must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if cmusicaid is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "cmusicaid"]) is not None:
            print("%sWARNING!%s There is already a cmusicaid process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "CMUSICAID" not in os.environ:
        os.environ["CMUSICAID"] = build_dir + '/src/cmusicaid' + exeext
        os.environ["CMUSICAICLI"] = build_dir + '/src/cmusicai-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `cmusicai-cli help` (`rpc_interface.txt`).
