...
AC_DEFUN([CMUSICAI_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build cmusicai-qt GUI (default=auto)])],
    [
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); cmusicai_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   CMUSICAI_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _CMUSICAI_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: cmusicai_enable_qt, cmusicai_enable_qt_dbus, cmusicai_enable_qt_test
AC_DEFUN([CMUSICAI_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  CMUSICAI_QT_CHECK([_CMUSICAI_QT_FIND_LIBS])

  dnl This is ugly and complicated. Yuck. Works as follows:
  dnl For Qt5, we can check a header to find out whether Qt is build
  dnl statically. When Qt is built statically, some plugins must be linked into
  dnl the final binary as well.
  dnl _CMUSICAI_QT_CHECK_STATIC_PLUGIN does a quick link-check and appends the
  ...
  if test "x$cmusicai_cv_static_qt" = xyes; then
    _CMUSICAI_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _CMUSICAI_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _CMUSICAI_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      ...
      _CMUSICAI_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _CMUSICAI_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _CMUSICAI_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  CMUSICAI_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  CMUSICAI_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  CMUSICAI_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  CMUSICAI_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  CMUSICAI_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  ...
  if test x$cmusicai_enable_qt = xyes; then
    AC_MSG_RESULT([$cmusicai_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$cmusicai_enable_qt])
  fi
...
dnl
dnl _CMUSICAI_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_CMUSICAI_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); CMUSICAI_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _CMUSICAI_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Inputs: no inputs.
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_CMUSICAI_QT_CHECK_STATIC_LIBS], [
...
AC_DEFUN([_CMUSICAI_QT_FIND_LIBS],[
  CMUSICAI_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [],
                      [CMUSICAI_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  CMUSICAI_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [],
                      [CMUSICAI_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  CMUSICAI_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [],
                      [CMUSICAI_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  CMUSICAI_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [],
                      [CMUSICAI_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
